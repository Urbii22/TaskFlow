...
def main():
    ...
    try:
        ...
        s.add_all([owner, assignee]); s.commit()
        s.refresh(owner); s.refresh(assignee)
        ...
        task = Task(title="Tarea", priority=TaskPriority.MEDIUM, position=1, column_id=col1.id, assignee_id=assignee.id)
        ...
        # Verificaciones
        s.refresh(owner); s.refresh(board); s.refresh(task)
        ...
        new_board = Board(name="Demo2", owner_id=owner.id); s.add(new_board); s.commit(); s.refresh(new_board)
        new_col = Column(name="Todo", position=1, board_id=new_board.id); s.add(new_col); s.commit(); s.refresh(new_col)
